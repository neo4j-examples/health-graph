...
def create_prescription_node(file, g):
    ...
    CREATE (pc:prescription {npi: row.npi, drugName: row.drug_name, genericName: row.neneric_name,
    totalCost: toFloat(row.total_drug_cost), speciality: row.specialty_description})
    FOREACH(ignoreME IN CASE WHEN trim(row.bene_count) <> "" THEN [1] ELSE [] END | SET pc.beneCount = row.bene_count)
    FOREACH(ignoreME IN CASE WHEN trim(row.total_claim_count) <> "" THEN [1] ELSE [] END | SET pc.totalCalimCount = row.total_claim_count)
    FOREACH(ignoreME IN CASE WHEN trim(row.total_drug_cost) <> "" THEN [1] ELSE [] END | SET pc.totalDrugCost = row.total_drug_cost)
    FOREACH(ignoreME IN CASE WHEN trim(row.bene_count_ge65) <> "" THEN [1] ELSE [] END | SET pc.beneCountAge65 = row.bene_count_ge65)
    FOREACH(ignoreME IN CASE WHEN trim(row.total_claim_count_ge65) <> "" THEN [1] ELSE [] END | SET pc.totalClaimCountAge65 = row.total_claim_count_ge65)
    FOREACH(ignoreME IN CASE WHEN trim(row.total_drug_cost_ge65) <> "" THEN [1] ELSE [] END | SET pc.totalDrugCostAge65 = row.total_drug_cost_ge65)
    '''
